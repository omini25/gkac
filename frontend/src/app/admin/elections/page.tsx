...
function ResultsView({ results, onClose, setLightbox }: { results: ElectionResults; onClose: () => void; setLightbox: (v: { src: string; alt: string } | null) => void }) {
  ...
  function exportResultsPdf() {
    ...
    doc.setFontSize(18);
    doc.setFont("helvetica", "bold");
    doc.text(results.election.title, pageWidth / 2, y, { align: "center" });
    y += 8;

    // Subtitle
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    const dateRange = results.election.startDate && results.election.endDate
      ? `${new Date(results.election.startDate).toLocaleDateString("en-GB")} – ${new Date(results.election.endDate).toLocaleDateString("en-GB")}`
      : "";
    if (dateRange) {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(`Status: ${results.election.status.toUpperCase()}`, pageWidth / 2, y, { align: "center" });
    y += 5;
    doc.text(`Generated: ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" })}`, pageWidth / 2, y, { align: "center" });
    y += 10;
    ...
    for (const pos of results.positions) {
      ...
      const winnerLabel = results.election.status === "closed" ? "WINNER" : "LEADING";
      ...
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text(pos.title, margin, y);
      y += 5;
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      doc.text(`${totalVotes} vote${totalVotes !== 1 ? "s" : ""}`, margin + 4, y);
      y += 6;
      ...
      for (const c of sorted) {
        ...
        const votes = `${c.voteCount} vote${c.voteCount !== 1 ? "s" : ""} (${c.percentage}%)`;
        const badge = isWinner ? `  [${winnerLabel}]` : "";
        doc.text(`• ${name}${badge}`, margin + 4, y);
        doc.text(votes, pageWidth - margin, y, { align: "right" });
        y += 4;

        // Progress bar (simple line)
        if (totalVotes > 0 && c.percentage > 0) {
          const barWidth = 60;
          const fillWidth = (c.percentage / 100) * barWidth;
          doc.setDrawColor(220, 220, 220);
          doc.setFillColor(isWinner ? (results.election.status === "closed" ? 76 : 255) : 230, isWinner ? (results.election.status === "closed" ? 175 : 200) : 230, isWinner ? (results.election.status === "closed" ? 80 : 50) : 230);
          doc.roundedRect(margin + 4, y + 1, barWidth, 4, 2, 2, "F");
          if (fillWidth > 0) {
            doc.setFillColor(isWinner ? (results.election.status === "closed" ? 76 : 255) : 200, isWinner ? (results.election.status === "closed" ? 175 : 193) : 200, isWinner ? (results.election.status === "closed" ? 80 : 7) : 200);
            doc.roundedRect(margin + 4, y + 1, fillWidth, 4, 2, 2, "F");
          }
          y += 8;
        } else {
          y += 4;
    ...
    doc.setTextColor(150);
    doc.text("GKAC Election Results — Generated by GKAC Admin", pageWidth / 2, doc.internal.pageSize.getHeight() - 10, { align: "center" });

    doc.save(`election-results-${results.election.title.replace(/\s+/g, "-").toLowerCase()}-${new Date().toISOString().slice(0, 10)}.pdf`);
  }
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn btn-outline btn-sm" onClick={exportResultsPdf}>📄 Export PDF</button>
